...
class MrpProductionRequestCreateMo(models.TransientModel):
    ...
    def compute_product_line_ids(self):
        ...
        for line in product_lines:
            self.env["mrp.request.create.mo.line"].create(
                self._prepare_product_line(line)
            )
        self._get_mo_qty()
        # The wizard must be reloaded in order to show the new product lines
        action = self.env.ref("mrp_request.mrp_request_create_mo_action")
        res = action.read()[0]
        res["res_id"] = self.id
        return res
    ...
    def _prepare_lines(self):
        ...
        factor = self.mrp_request_id.product_uom_id._compute_quantity(
            self.pending_qty, bom_point.product_uom_id
        )
        return bom_point.explode(
            self.mrp_request_id.product_id, factor / bom_point.product_qty
        )
    ...
    mrp_request_id = fields.Many2one(comodel_name="mrp.request", readonly=True)
    bom_id = fields.Many2one(related="mrp_request_id.bom_id", readonly=True)
    mo_qty = fields.Float(string="Quantity", digits="Product Unit of Measure")
    pending_qty = fields.Float(
        related="mrp_request_id.pending_qty",
        digits="Product Unit of Measure",
    )
    product_uom_id = fields.Many2one(related="mrp_request_id.product_uom_id")
    product_line_ids = fields.One2many(
        comodel_name="mrp.request.create.mo.line",
        string="Products needed",
        inverse_name="mrp_request_create_mo_id",
        readonly=True,
    )
    date_start = fields.Datetime(string="Deadline Start", required=True)
    date_finished = fields.Datetime(string="Deadline End", required=True)
    ...
    def _prepare_product_line(self, pl):
        return {
            ...
            "mrp_request_create_mo_id": self.id,
            "location_id": self.mrp_request_id.location_src_id.id,
        }

    def _prepare_manufacturing_order(self):
        self.ensure_one()
        request_id = self.mrp_request_id
        return {
...
class MrpProductionRequestCreateMoLine(models.TransientModel):
    _name = "mrp.request.create.mo.line"
    ...
    mrp_request_create_mo_id = fields.Many2one(comodel_name="mrp.request.create.mo")
